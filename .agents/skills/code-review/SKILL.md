---
name: code-review
description: Independent semantic review protocol for exact UV Studio PR diffs before merge. Use from a fresh ordinary-ChatGPT review context to find concrete correctness, security, recovery, concurrency, identity, authority, Product Truth and acceptance defects, falsify candidate findings, and bind the result to exact BASE_SHA/HEAD_SHA. This review is read-only and separate from implementation.
...
Optional informational fields may identify required physical gates or a known change class, but they must not contain a developer argument that the change is correct, proposed findings or a reasoning summary.
...
When the PR itself introduces `code-review` for the first time, the prior accepted `AGENTS.md` / `DEVELOPMENT_PROTOCOL.md` merge policy governs adoption; after merge, this skill becomes the primary semantic-review protocol for later applicable PRs.
...
- current architecture/security/Product Truth/acceptance owners when the diff touches their invariants;
...
1. correctness / wrong result / broken domain or API contract;
2. security / D-017 authorization / trust-boundary widening / secret exposure;
3. ambiguous side effects / recovery / retry / reconciliation / replay;
4. concurrency / shared-project fencing / duplicate effects / stale ownership / idempotency;
5. identity / provenance / ABA-style replacement / stale Scene-Shot-Take-asset-Job-Timeline binding;
6. persistence / Project Store / Production / Timeline / Job / Task crash and restart semantics;
7. authority bypasses around Studio/Application Commands, `ProjectUnitOfWork`, Generation, Capability Registry or canonical Timeline/Production state;
8. verification mistakes including false `PASS`, stale evidence, source/runtime provenance gaps or acceptance bypasses;
9. Product Truth/current-doc/backend/frontend/E2E disagreement for user-visible work;
10. lifecycle / exact-head / merge-base / review-provenance mistakes;
11. cleanup / resource lifecycle / liveness;
12. test gaps that allow a **concrete regression introduced by this diff** to pass.

Do not report style, naming preference, refactoring taste, speculative future architecture or generic best-practice advice unless it creates a concrete correctness/security/recovery/authority/acceptance failure in the reviewed change.

## 6. Build candidate findings, then try to disprove them

A suspicious pattern is only a `CandidateFinding`.

For every candidate, gather enough evidence to answer:
...
- inspect neighboring code and all materially relevant callers;
- inspect relevant tests, fixtures and canonical owner documents;
- look for guards, validation, locking, idempotency, recovery or invariants that defeat the scenario;
...
- check whether an existing canonical authority already prevents the bypass or stale write;
...
fresh reviewer
 ...
 -> rejected finding: record contrary evidence / no code change
...
- production/runtime/frontend behavior;
- security or authorization policy;
- persistence/recovery/retry/reconciliation;
- concurrency/identity/ownership/provenance;
- canonical Project/Production/Timeline/Generation authority;
- verification or acceptance/Finish Gate semantics;
...
- Product Truth/current-authority claims that alter accepted readiness;
...
fresh required ChatGPT review on exact BASE_SHA..HEAD_SHA
 ...
 -> verify unresolved review conversations/findings are empty
 -> merge
```
