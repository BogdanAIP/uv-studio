...
import { ProductRenderPanel } from './ProductRenderPanel';
...
export function ProjectExportWorkspace({
  ...
  if (!state) {
    return <div className="rounded-2xl border border-[var(--uv-border)] bg-[var(--uv-surface-0)] p-6 text-sm text-zinc-500">{error ?? 'Загрузка результатов…'}</div>;
  }
    ...
    <div className="space-y-4">
      <section className="flex flex-col gap-4 rounded-2xl border border-[var(--uv-border)] bg-[var(--uv-surface-0)] p-5 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-500">Соберите применённые изменения в итоговое видео или сохраните весь проект переносимым архивом.</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <button type="button" onClick={() => void manualRefresh()} disabled={refreshing} className="inline-flex h-10 items-center gap-2 rounded-xl border border-[var(--uv-border)] bg-[var(--uv-surface-1)] px-3 text-sm text-zinc-400 transition hover:text-zinc-200 disabled:opacity-40">
            <RefreshCw size={15} className={refreshing ? 'animate-spin' : ''} /> Обновить
          </button>
          <a href={archiveUrl} download className="inline-flex h-10 items-center gap-2 rounded-xl border border-[var(--uv-border-strong)] bg-[var(--uv-surface-1)] px-3 text-sm text-zinc-300 transition hover:bg-[var(--uv-surface-2)]">
            <Download size={15} /> Архив проекта
          </a>
        </div>
      </section>
        ...
        <>
          {videos.length > 1 && (
            <label className="block max-w-md text-xs text-zinc-600">
              Исходное видео
              <select value={selectedSource.id} onChange={event => setSelectedSourceId(event.target.value)} className="mt-2 w-full rounded-xl border border-[var(--uv-border)] bg-[var(--uv-surface-1)] px-3 py-2.5 text-sm text-zinc-300">
                {videos.map(source => <option key={source.id} value={source.id}>{sourceName(source)}</option>)}
              </select>
            </label>
          )}
          <ProductRenderPanel projectId={projectId} editorState={state} source={selectedSource} onStateChanged={refresh} onProjectChanged={onProjectChanged} />
        </>
      ) : null}
    </div>
  );
