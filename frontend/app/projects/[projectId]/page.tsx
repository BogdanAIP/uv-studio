...
import { SequenceContinuityWorkspace } from '@/components/editor/SequenceContinuityWorkspace';
...
export default function ProjectPage() {
    ...
    return (
      <main className="min-h-screen p-6 sm:p-8">
        <div className="mx-auto max-w-3xl rounded-2xl border border-rose-400/20 bg-rose-400/10 p-6 text-rose-200">
          ...
          <Link href="/projects" className="mt-5 inline-flex items-center gap-2 text-sm text-rose-100 underline underline-offset-4"><ArrowLeft size={15} /> Вернуться к проектам</Link>
        </div>
      </main>
    );
  }
  ...
  if (!project || !executionPlan) {
    return <main className="min-h-screen p-6 sm:p-8"><div className="mx-auto max-w-6xl animate-pulse rounded-2xl border border-[var(--uv-border)] bg-[var(--uv-surface-0)] p-8 text-sm text-zinc-600">Открываем проект…</div></main>;
  }
  ...
  const tabs: Array<{ id: WorkspaceId; label: string; icon: typeof Film }> = [{ id: 'edit', label: 'Монтаж', icon: Film }];
    ...
    <main className="min-h-screen bg-[var(--uv-bg)]">
      <header className="sticky top-0 z-30 border-b border-[var(--uv-border)] bg-[rgba(9,10,13,0.9)] px-4 backdrop-blur-xl sm:px-6">
        <div className="flex min-h-16 items-center gap-3">
          <Link href="/projects" aria-label="Все проекты" className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg text-zinc-600 transition hover:bg-white/[0.04] hover:text-zinc-200"><ArrowLeft size={17} /></Link>
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-0.5 text-[10px] text-zinc-700">Локальный проект · рабочее состояние сохраняется внутри проекта</p>
          </div>
          <AvailabilityBadge compatibility={executionPlan.compatibility} />
          <a href={projectArchiveUrl(project.project_id)} download={`${project.project_id}.uvproj.zip`} title="Скачать архив проекта" className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-[var(--uv-border)] bg-[var(--uv-surface-1)] text-zinc-500 transition hover:text-zinc-200"><Download size={15} /></a>
        </div>
        ...
        <nav className="flex gap-1 overflow-x-auto pb-2" aria-label="Рабочие разделы проекта">
          {tabs.map(tab => {
            ...
            return (
              <button key={tab.id} type="button" data-testid={`workspace-${tab.id}`} onClick={() => setWorkspace(tab.id)} className={`inline-flex h-9 shrink-0 items-center gap-2 rounded-lg px-3 text-xs font-medium transition ${active ? 'bg-violet-400/12 text-violet-200 ring-1 ring-inset ring-violet-400/20' : 'text-zinc-600 hover:bg-white/[0.035] hover:text-zinc-300'}`}>
      ...
      <div className="mx-auto max-w-[1760px] p-3 sm:p-5 lg:p-6">
        {executionPlan.compatibility !== 'available' && workspace === 'edit' && <AvailabilityNotice compatibility={executionPlan.compatibility} />}
        {workspace === 'edit' && <ProjectEditor projectId={project.project_id} onProjectChanged={refreshProject} />}
        {workspace === 'task' && hasTask && <TaskWorkspace project={project} workflowRefresh={workflowRefresh} refreshProject={refreshProject} refreshMusicPrerequisites={refreshMusicPrerequisites} />}
        {workspace === 'dubbing' && hasVideo && <DubbingWorkspace projectId={project.project_id} onProjectChanged={refreshProject} />}
        {workspace === 'continuity' && hasVideo && <WorkspaceFrame eyebrow="Связанные кадры" title="Связность сцен" description="Используйте этот инструмент только там, где следующий кадр должен продолжать принятый предыдущий."><SequenceContinuityWorkspace projectId={project.project_id} onProjectChanged={refreshProject} /></WorkspaceFrame>}
        {workspace === 'export' && hasVideo && <ProjectExportWorkspace projectId={project.project_id} archiveUrl={projectArchiveUrl(project.project_id)} onProjectChanged={refreshProject} />}
      </div>
    </main>
  );
}
...
function AvailabilityBadge({ compatibility }: { compatibility: ProjectExecutionPlan['compatibility'] }) {
  if (compatibility === 'available') return <span className="hidden rounded-full bg-emerald-400/10 px-2.5 py-1 text-[10px] text-emerald-300 sm:inline">Готово к работе</span>;
  if (compatibility === 'partial') return <span className="hidden rounded-full bg-amber-400/10 px-2.5 py-1 text-[10px] text-amber-200 sm:inline">Часть функций требует настройки</span>;
...
function AvailabilityNotice({ compatibility }: { compatibility: ProjectExecutionPlan['compatibility'] }) {
  return <div className="mb-4 flex items-start gap-3 rounded-xl border border-amber-400/15 bg-amber-400/[0.06] px-4 py-3 text-xs leading-5 text-zinc-500"><WandSparkles size={16} className="mt-0.5 shrink-0 text-amber-300" /><span>{compatibility === 'partial' ? 'Часть автоматических действий для этого типа проекта требует подключения. Монтаж, материалы и локальная работа доступны сейчас.' : 'Автоматическое выполнение этого шаблона пока не подключено. Можно импортировать материалы, монтировать и использовать доступные инструменты без подмены задачи другим режимом.'}</span></div>;
}
...
function WorkspaceFrame({ eyebrow, title, description, children }: { eyebrow: string; title: string; description: string; children: React.ReactNode }) {
  return <div className="space-y-4"><section className="rounded-2xl border border-[var(--uv-border)] bg-[var(--uv-surface-0)] p-5"><p className="text-xs font-medium uppercase tracking-[0.18em] text-zinc-600">{eyebrow}</p><h2 className="mt-2 text-xl font-medium text-zinc-100">{title}</h2><p className="mt-1 max-w-3xl text-sm leading-6 text-zinc-500">{description}</p></section>{children}</div>;
}

function TaskWorkspace({ project, workflowRefresh, refreshProject, refreshMusicPrerequisites }: { project: UVProject; workflowRefresh: number; refreshProject: () => Promise<void>; refreshMusicPrerequisites: () => Promise<void> }) {
  ...
  if (project.recipe_id === 'story_video' || project.recipe_id === 'commercial_product' || project.recipe_id === 'free_project') return <WorkspaceFrame eyebrow="Подготовка" title={TASK_LABELS[project.recipe_id] ?? 'Материалы'} description="Задача и исходные материалы этого проекта. Монтаж использует те же данные проекта."><Stage8CompositionPanel projectId={project.project_id} recipeId={project.recipe_id} sources={project.sources} onProjectChanged={refreshProject} /></WorkspaceFrame>;
  if (project.recipe_id === 'photo_to_video' || project.recipe_id === 'visualizer') return <WorkspaceFrame eyebrow="Локальная сборка" title={TASK_LABELS[project.recipe_id]} description="Сборка выполняется локально из материалов этого проекта без обязательного внешнего сервиса."><Stage8MediaPanel projectId={project.project_id} recipeId={project.recipe_id} sources={project.sources} onProjectChanged={refreshProject} /></WorkspaceFrame>;
  if (project.recipe_id === 'performance_lip_sync') return <WorkspaceFrame eyebrow="Performance" title="Lip-sync" description="Подготовьте персонажа и речь, затем проверьте результат перед использованием."><PerformanceLipSyncPanel projectId={project.project_id} sources={project.sources} onProjectChanged={refreshProject} /></WorkspaceFrame>;
  return null;
}
...
function DubbingWorkspace({ projectId, onProjectChanged }: { projectId: string; onProjectChanged: () => Promise<void> }) {
  ...
  return <WorkspaceFrame eyebrow="Речь и язык" title="Дубляж" description="Распознавание, перевод, подготовленная речь и экспорт субтитров собраны в одном инструменте и показываются по шагам.">
