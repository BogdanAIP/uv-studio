...
3. `recipe-entrypoint-retirement` — exact-scan `frontend/lib/recipesApi.ts`, migrate any supported caller or delete that client with `/api/uv/recipes`, then retire the backend entrypoint subject to persisted-project compatibility;
...
Exact-head Codex review has found concrete documentation/inventory omissions around the supported Settings/modelRegistry caller of `workflowApi.fetchApiModels`, donor frontend restoration authority, live Stage8 runtime callers, D-070 gate semantics, current-authority lifecycle wording, no-force/Windows restore guidance, promotion unit-test/CI callers, and the exported frontend clients for `/api/uv/recipes` and `/execution-plan`. All fixes remain documentation/context-only; the corrected final head must receive a fresh full permanent CI pass and exact-head review before merge.
...
Later recipe/execution endpoint retirement is separately gated on exact-scanning the exported `recipesApi.ts` and `projectsApi.getProjectExecutionPlan()` client seams; those are not part of `donor-ui-retirement`.
