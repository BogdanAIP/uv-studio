...
## Seventh Draft repair in progress

### P2 — preserve redo-owned managed media during archive export

The post-Ready reviewer reproduced a valid undo state where `generation.register_output` and `production.register_take` have both been undone while their Redo branch remains reachable. Startup recovery already preserves the generated bytes through the durable UOW redo suffix, but `_reject_unpublished_managed_media()` currently compares self-identifying media only with live Project references and therefore rejects archive export of that valid recoverable state.

The repair must make archive validation consult the same durable redo authority, while still rejecting genuine unregistered managed media once Redo is truncated. Regression coverage must prove `two Undo -> export/import -> two Redo` preserves exact generated bytes and historical artifact/Take identity, and must retain the existing genuine-orphan fail-closed behavior.

## Last accepted verification

Exact frozen review head `4b8ec3492b3c19e897bfb09421c68d1a945e9e5e` passed post-Ready CI #4381 (`33554207955`) **5/5** before the new archive finding appeared:
...
That CI is now stale for merge authority because the seventh repair will change material code/tests.
...
1. implement the narrow archive redo-authority repair and deterministic regression;
2. synchronize PR body/context and obtain exact Draft-head CI **5/5**;
3. reply to and resolve the new inline P2 with concrete evidence;
4. re-resolve live repository/base/head/thread identity;
5. context-only refreeze `draft -> review`;
6. return PR #89 to **Ready for review** without changing the frozen Git head;
7. require authoritative post-Ready exact-head CI **5/5**;
8. run a completely fresh ordinary-ChatGPT semantic review under governing BASE `.agents/skills/code-review/SKILL.md` v1.0 against the exact frozen BASE..HEAD;
9. merge only if the fresh result is `CURRENT`, reports zero findings, exact-head CI remains green, base/head remain unchanged and zero unresolved review threads remain.
