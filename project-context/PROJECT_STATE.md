...
- Performance/lip-sync remains fail-closed unless the exact optional MuseTalk pack satisfies D-043 checkout, shadow-code, model-payload, runtime and CUDA verification.
...
5. **Performance / Lip-sync** — supplied portrait + finished speech through optional local MuseTalk 1.5 offer `local_musetalk.video_digital_human`. The offer becomes executable only after exact pinned checkout, tracked-worktree, untracked-shadow, entrypoint fingerprint, exact binary model SHA-256, runtime import and CUDA checks pass; otherwise the product reports `configuration_required`/partial and creates no fake result.
