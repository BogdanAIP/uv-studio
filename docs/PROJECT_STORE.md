...
2. WebVTT `sub_<uuid>.vtt` and named Generation `generated_attempt_<uuid>.*` outputs are self-identifying UV-owned publications. If a crash leaves those bytes without a ProjectReference, startup reconciliation moves them outside the project tree and archive export fails closed until that split state is resolved. Ordinary unregistered project files with unrelated names remain portable.
3. Generation has additional durable boundaries after the artifact ProjectReference is committed. If restart finds the exact running Job/Attempt already bound by artifact provenance, recovery does not replay the provider. It validates the artifact bytes, reuses an existing matching Take or creates the missing Take through the normal Production command, then records Job success. If no durable artifact reference exists, the orphan generated bytes are quarantined and the abandoned running Job becomes failed/retryable. Archive export separately rejects a generation artifact whose durable Job has not reached matching success.
...
Source upload keeps its established boundary: request-body streaming/staging remains outside the fence, while final move, FFprobe validation, portable metadata derivation and source registration remain inside it. The historical archive-side managed-name detector remains a fail-closed fallback for incomplete source/artifact/audio publications.
