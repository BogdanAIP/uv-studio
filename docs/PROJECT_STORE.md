...
Redo authority is an exact reachable state chain rather than a bag of historical `after` snapshots. From the current cursor, `redo_project_documents()` simulates every changed canonical document in order, requires each recorded `before` snapshot to equal the simulated current state, then applies its `after` snapshot. A structurally valid journal whose next `before` state is unreachable therefore cannot become archive/startup preservation authority.
...
Generation ProjectReference state has an additional reserved persistence boundary. If `metadata.generation` is present it must be a typed JSON object with safe Job/Attempt identifiers, a generation contract and continuation lineage consistent with that contract. A Generation reference may not live in `Project.sources`; Generation ownership remains artifact authority. An artifacts-root Generation path must be a direct `artifacts/generated_<attempt>...` file rather than a nested path. Once a Generation reference ID already exists, generic canonical mutation may add unrelated metadata such as Production acceptance bindings, but while that ID remains present it may not change path/kind or strip/rebind the reserved `metadata.generation` authority. Undo may still remove the reference entirely, and Redo re-addition is independently revalidated against durable Job/Attempt and exact bytes.

Because ProjectUnitOfWork snapshots canonical JSON rather than binary media, Undo of `generation.register_output` deliberately leaves the already-published generated bytes in place while that transaction remains reachable through the current Redo suffix. That temporary binary preservation is not pathname authority. The validated redo `project.json` snapshots retain the full historical Generation ProjectReference, and its durable Job/Attempt/provenance plus persisted `size_bytes` and SHA-256 remain mandatory. Archive export and startup recovery permit ordinary metadata evolution across historical versions of the same reference only when ID/path/kind and immutable Generation authority remain the same; path reuse, role/classification drift or Generation provenance drift fail closed. A later canonical commit truncates the redo suffix and removes the protection, after which the same unregistered managed bytes become ordinary orphan candidates again.

Direct Redo validates the Generation references that would remain live after **every** Redo operation, not only Redos that themselves replace `project.json`. Therefore a valid artifact Redo followed by byte substitution cannot be followed by a Production-only Redo that restores a Take around altered media.
...
Current root staging for Generation, source upload, WebVTT, FFconcat and `timeline.assemble` is lease-backed. Allocation creates a sidecar `<staging-name>.lease` and holds a one-byte OS lock from staging reservation through ordinary final cleanup without pre-creating the producer output path. On hard process loss the operating system releases that lock. Application startup, before project-scoped publication/Generation recovery, scans only the Project Store root and removes only exact current UV-owned staging names whose lease can be acquired non-blockingly. A still-locked lease proves another cooperating runtime is actively using the staging path and is preserved. Unknown or malformed root files, project directories, recovery quarantine files, symlinks/non-files and exact-looking legacy staging that has no lease are not guessed stale or recursively deleted. Root staging and its lease are transient coordination state outside canonical project authority and outside portable project archives.
