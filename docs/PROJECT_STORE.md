...
1. `timeline.assemble` can publish to an arbitrary caller-selected `artifacts/` or `exports/` path. Immediately before its canonical `os.replace`, while already holding the shared project fence, it writes a small `pub_<uuid>` managed-publication marker under `tasks/`. Normal completion removes the marker only after the ProjectReference is durable. If the process dies, archive export refuses the project while the marker remains; startup reconciliation clears a stale marker for an already registered path or moves unregistered bytes out of the canonical project tree before clearing it.
...
3. Generation has additional durable boundaries after the artifact ProjectReference is committed. Every consequence-bearing Generation Job transition now participates in the same cross-runtime project fence, so cancel/fail/retry cannot interleave with final publication. Once an artifact is durable, failure/cancellation of that current attempt is refused until reconciliation. If local Take or Job-success persistence fails after artifact commit, the Job therefore remains recoverable rather than being stranded terminally. Startup recovery can also repair a legacy current-attempt `FAILED`/`CANCELLED` split state produced by an older implementation. Before creating/reusing a Take or recording success, recovery verifies the artifact's exact persisted size, SHA-256 and Job/Attempt/model/execution-mapping/request/contract provenance against the live canonical file and durable Job. No provider work is replayed. If no durable artifact reference exists, orphan generated bytes are quarantined and an abandoned running Job becomes failed/retryable. Retry is rejected while the current failed attempt still has a durable artifact pending reconciliation.

Archive export independently revalidates Generation success/provenance and compares the SHA-256 and size computed from the exact bytes streamed into the ZIP with the persisted Generation artifact digest/size. A changed, truncated or substituted generated file therefore cannot become a successful recovery result or a portable archive merely because it remains non-empty.

`ProjectUnitOfWork` recovery runs before publication reconciliation so a half-applied prepared Project transaction is rolled back to its exact snapshot before artifact/Take/Job crash state is classified.

Source upload keeps its established execution boundary: request-body streaming/staging remains outside the fence, while final move, FFprobe validation, portable metadata derivation and source registration remain inside it. Because a hard process loss can bypass ordinary exception cleanup after the final move, startup now explicitly quarantines an unregistered canonical `sources/src_<uuid>.*` before archive retry. The historical archive-side managed-name detector remains a fail-closed fallback before reconciliation.
