...
4. reconstructs full redo-owned source/artifact ProjectReferences, accepting canonical metadata evolution only when stable identity/path/kind and immutable Generation authority agree and otherwise failing closed;
...
10. validates every live or redo-only Generation artifact against its own durable successful attempt, current Production Take authority or exact durable Undo evidence, matching Job/request/provenance/lineage and persisted size/SHA-256;
