...
- **final deletion proof** — the retirement PR must run an exact recursive import/reference scan from its checkout, prove no enabled workflow, developer tool, setup script, or documented supported recovery path can restore the deleted donor surfaces, then lint + production build + permanent browser/API CI after deletion.
...
| donor frontend restoration (`.github/workflows/promote-frontend.yml`, `tools/promote_frontend.py`, `tools/uv_dev.py`, `scripts/setup-dev.ps1`, `docs/FRONTEND.md`) | **ADAPT → DELETE LATER** | workflow dispatch uses `promote_frontend.py --force` to replace maintained frontend bytes; plain `promote_frontend.py` recreates pinned donor bytes when `frontend/` is absent; `uv_dev.py` and Windows `setup-dev.ps1` actively recommend that plain restore for missing frontend files; frontend support docs describe the promotion/provenance mechanism | provenance verification/check-only path with no write authority over maintained `frontend/`; explicit archival/reconstruction flow outside the maintained product branch only if still required | disable/remove/replace every supported write-capable restoration path and update its workflow/tool/setup/documentation callers before donor deletion; prove no supported recovery path can restore retired donor files; retain provenance checking only if read-only with respect to `frontend/` |
...
- supported repository/developer/setup recovery can currently recreate the entire donor frontend through both force and no-force promotion paths, so all write-capable restoration entry points and their guidance must be removed/replaced before deletion is durable;
