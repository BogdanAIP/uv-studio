...
- **frontend endpoint-client compatibility edges** — `frontend/lib/recipesApi.ts` exports list/get clients for `/api/uv/recipes`, and `frontend/lib/projectsApi.ts` exports `getProjectExecutionPlan()` for `/execution-plan`. This inventory has not established a positive supported import for those exports, but incomplete Code Search is not absence proof; the endpoint-retirement PRs must exact-scan the checkout and either migrate any discovered caller or delete these client exports with their endpoints;
...
3. **`recipe-entrypoint-retirement`** — move remaining modern/creation callers from Recipe Registry and `/api/uv/recipes`; exact-scan `frontend/lib/recipesApi.ts` and migrate any real caller or delete that client with the endpoint; retain only bounded compatibility metadata if still required.
