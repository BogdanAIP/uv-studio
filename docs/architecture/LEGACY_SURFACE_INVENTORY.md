...
| Stage 8 workspace state/API (`projects.stage8_workspace`, `api/stage8_workspace.py`) | **LEGACY** | live legacy panels + `test_stage8_workspace_api.py` and Stage8 browser/API outcomes | direction-specific/shared input documents + project media/contextual tools | persisted Stage8 projects migrated/read compatibly; legacy page no longer uses Stage8 workspace; tests moved |
...
D-070 names “Stage 6/8” because earlier architecture used numbered workspace layers. On the current baseline, Stage 8 has concrete surviving code (`api/stage8_workspace.py`, project Stage8 workspace state and live compatibility panels/tests).
...
7. **`stage8-compatibility-retirement`** — remove Stage8 workspace/API/editor compatibility only after persisted projects migrate or have a supported reader path.
8. **`product-orchestrator-retirement`** — remove final recipe dispatch/projection only when no supported route/API/test depends on it.
