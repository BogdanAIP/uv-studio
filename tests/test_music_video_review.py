...
class MusicVideoReviewTests(unittest.TestCase):
    def _reference(self, store, project_id, *, ref_id, kind, path, payload, duration_us, artifact=False, metadata=None):
        ...
        if artifact:
            store.update_project(project_id, artifacts=(*project.artifacts, ref))
        else:
            store.update_project(project_id, sources=(*project.sources, ref))
        return ref
    ...
    def _setup(self, tmp: str, *, excerpt_end_us: int = 20_000_000):
        ...
        music_map = MusicMapStore(store).set_map(
            ...
            excerpt=MusicExcerpt(0, excerpt_end_us),
            sections=(MusicSection("whole", "other", "Whole", 0, excerpt_end_us),),
        ...
        direction = MusicDirectionStore(store).set_direction(
            ...
            shots=(
                ...
                MusicShotPlan("b", 1, split, excerpt_end_us, "B"),
            ),
        )
        assembly = MusicAssemblyStore(store).set_assembly(
            pid,
        ...
        artifact = self._reference(
            ...
            payload=b"canonical-final-render",
            duration_us=excerpt_end_us,
            artifact=True,
            metadata={
    ...
    def test_approved_review_requires_and_records_exact_release_evidence(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            review = service.review(
                ...
                note="Transitions checked against the final render.",
            )
    ...
    def test_transition_failure_and_artifact_substitution_fail_closed(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store, pid, artifact = self._setup(tmp)
            service = MusicVideoReviewStore(store)
