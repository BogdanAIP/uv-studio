...
class MusicAnalysisAndFinalReviewApiTests(unittest.TestCase):
    ...
    def _project(self, title: str = "Music HTTP") -> str:
        response = self.client.post(
            "/api/uv/projects",
            json={"title": title, "recipe_id": "music_video"},
        )
    ...
    def _reference(
        ...
        *,
        ...
        if artifact:
            self.store.update_project(project_id, artifacts=(*project.artifacts, reference))
        else:
            self.store.update_project(project_id, sources=(*project.sources, reference))
        return reference

    def test_analysis_assist_normalizes_advice_without_writing_music_map(self) -> None:
        project_id = self._project("Analysis HTTP")
        song = self._reference(
            project_id,
            ref_id="song",
            kind="audio",
            path="sources/song.wav",
            payload=b"analysis-http-song",
            duration_us=30_000_000,
        )
        package = self.client.get(
            f"/api/uv/projects/{project_id}/music-analysis-assist",
            params={"song_reference_id": song.id},
        )
        ...
        normalized = self.client.post(
            f"/api/uv/projects/{project_id}/music-analysis-assist/normalize",
            params={"song_reference_id": song.id},
            json={
                "binding": body["binding"],
                "excerpt": {"start_us": 5_000_000, "end_us": 25_000_000},
                "sections": [
                    {
                        "section_id": "chorus",
                        "kind": "chorus",
                ...
                "markers": [
                    {"marker_id": "accent", "kind": "accent", "time_us": 15_000_000}
                ],
                "lyric_phrases": [],
                "note": "advisory",
            },
    ...
    def _review_fixture(self, *, duration_us: int) -> tuple[str, ProjectReference]:
        ...
        song = self._reference(
            ...
            kind="audio",
            path="sources/song.wav",
            payload=b"review-http-song",
            duration_us=max(duration_us, 30_000_000),
        )
        visual = self._reference(
            project_id,
            ref_id="visual",
            kind="video",
            path="sources/visual.mp4",
            payload=b"review-http-visual",
            duration_us=max(duration_us, 30_000_000),
        )
        split = duration_us // 2
        music_map = MusicMapStore(self.store).set_map(
            project_id,
            song_reference_id=song.id,
            excerpt=MusicExcerpt(0, duration_us),
            sections=(MusicSection("whole", "other", "Whole", 0, duration_us),),
            markers=(MusicTimingMarker("cut", "cut_point", split),),
        )
        direction = MusicDirectionStore(self.store).set_direction(
            project_id,
            music_map_revision_sha256=music_map.revision_sha256,
            shots=(
                MusicShotPlan("a", 0, 0, split, "A", ("cut",)),
                MusicShotPlan("b", 1, split, duration_us, "B"),
            ),
        )
        assembly = MusicAssemblyStore(self.store).set_assembly(
            project_id,
            music_direction_revision_sha256=direction.revision_sha256,
            assignments=(
                MusicVisualAssignment("a", visual.id, 0),
                MusicVisualAssignment("b", visual.id, split),
            ),
        )
    ...
    def test_final_review_http_enforces_release_gate_and_reopens_current_evidence(self) -> None:
        ...
        approved = self.client.post(
            f"/api/uv/projects/{project_id}/music-video-review",
            json={
                "artifact_id": artifact.id,
                "verdict": "approved",
                "transition_outcome": "pass",
                "note": "Transitions checked on final render.",
            },
        )
        ...
        self.assertEqual(
            reopened.json()["music_video_review"]["artifact_sha256"],
            artifact.metadata["sha256"],
        )

        short_id, short_artifact = self._review_fixture(duration_us=4_000_000)
        rejected = self.client.post(
            f"/api/uv/projects/{short_id}/music-video-review",
            json={
                "artifact_id": short_artifact.id,
                "verdict": "approved",
                "transition_outcome": "pass",
                "note": None,
            },
