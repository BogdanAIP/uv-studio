...
MCP_PROJECT_FILE_OUTPUT_SCHEMA_VERSION = 1
MCP_PROJECT_FILE_ALLOWED_ROOTS = frozenset(("sources", "assets", "artifacts", "exports"))
MCP_PROJECT_OUTPUT_MEDIA_KINDS = frozenset(
    (MediaKind.VIDEO, MediaKind.AUDIO, MediaKind.IMAGE, MediaKind.SUBTITLE)
)
...
class MCPProfile:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self,
            "title",
            _clean_text(self.title, field_name="title", max_length=200),
        )
        object.__setattr__(
            self,
            "command",
            _clean_text(self.command, field_name="command", max_length=2000),
        )
        if not isinstance(self.args, tuple):
            raise MCPConfigurationError("args must be a tuple")
        normalized_args = tuple(
            _clean_text(value, field_name="arg", max_length=4000)
        ...
        for pair in self.env_refs:
            if not isinstance(pair, tuple) or len(pair) != 2:
                raise MCPConfigurationError(
                    "each env_refs entry must be a (child, source) tuple"
                )
            child, source = pair
            if not isinstance(child, str) or not _ENV_RE.fullmatch(child):
                raise MCPConfigurationError(
                    f"invalid child environment name: {child!r}"
                )
            if not isinstance(source, str) or not _ENV_RE.fullmatch(source):
                raise MCPConfigurationError(
                    f"invalid source environment name: {source!r}"
                )
            if child in seen_children:
                raise MCPConfigurationError(
                    f"duplicate child environment name: {child!r}"
                )
    ...
    def from_dict(cls, data: Mapping[str, Any]) -> "MCPProfile":
        ...
        if not isinstance(raw_refs, Mapping):
            raise MCPConfigurationError(
                "env_refs must be an object mapping child name to source env name"
            )
...
class MCPProjectFileInput:
    """One explicit top-level MCP argument resolved from portable project storage."""
    ...
    def __post_init__(self) -> None:
        if self.schema_version != MCP_PROJECT_FILE_INPUT_SCHEMA_VERSION:
            raise MCPConfigurationError(
                "MCPProjectFileInput only supports schema "
                f"v{MCP_PROJECT_FILE_INPUT_SCHEMA_VERSION}"
            )
        object.__setattr__(
            self,
            "argument_name",
            _clean_text(self.argument_name, field_name="argument_name", max_length=256),
        )
        ...
        roots = tuple(
            _clean_text(root, field_name="allowed_root", max_length=64)
        ...
        if unknown:
            raise MCPConfigurationError(
                f"unsupported project-file roots: {sorted(unknown)!r}; "
                f"allowed roots are {sorted(MCP_PROJECT_FILE_ALLOWED_ROOTS)!r}"
            )
    ...
    def from_dict(cls, data: Mapping[str, Any]) -> "MCPProjectFileInput":
        ...
        _exact_keys(
            data,
            {"schema_version", "argument_name", "allowed_roots", "required"},
            context="project_file_input",
        )
        roots = data.get("allowed_roots", [])
        if not isinstance(roots, list):
            raise MCPConfigurationError(
                "project_file_input.allowed_roots must be an array"
            )
        return cls(
            schema_version=int(
                data.get("schema_version", MCP_PROJECT_FILE_INPUT_SCHEMA_VERSION)
            ),
...
@dataclass(frozen=True)
class MCPProjectFileOutput:
    """One binding-owned output argument backed by an allocated UV Studio path."""

    argument_name: str
    media_kind: MediaKind
    suffix: str
    required: bool = True
    schema_version: int = MCP_PROJECT_FILE_OUTPUT_SCHEMA_VERSION

    def __post_init__(self) -> None:
        if self.schema_version != MCP_PROJECT_FILE_OUTPUT_SCHEMA_VERSION:
            raise MCPConfigurationError(
                "MCPProjectFileOutput only supports schema "
                f"v{MCP_PROJECT_FILE_OUTPUT_SCHEMA_VERSION}"
            )
        object.__setattr__(
            self,
            "argument_name",
        ...
        media_kind = _enum_value(
            self.media_kind,
            MediaKind,
            field_name="project_file_output.media_kind",
        )
        if media_kind not in MCP_PROJECT_OUTPUT_MEDIA_KINDS:
            raise MCPConfigurationError(
                "project_file_output.media_kind must be one of "
                f"{sorted(item.value for item in MCP_PROJECT_OUTPUT_MEDIA_KINDS)!r}"
            )
        object.__setattr__(self, "media_kind", media_kind)
        if not isinstance(self.suffix, str) or not _SUFFIX_RE.fullmatch(self.suffix):
            raise MCPConfigurationError(
                "project_file_output.suffix must be a short extension such as '.mp4'"
            )
    ...
    @classmethod
    def from_dict(cls, data: Mapping[str, Any]) -> "MCPProjectFileOutput":
        if not isinstance(data, Mapping):
            raise MCPConfigurationError("project_file_output must be an object")
        _exact_keys(
            data,
            {"schema_version", "argument_name", "media_kind", "suffix", "required"},
            context="project_file_output",
        )
        return cls(
            schema_version=int(
                data.get("schema_version", MCP_PROJECT_FILE_OUTPUT_SCHEMA_VERSION)
            ),
            argument_name=data.get("argument_name"),
...
class MCPToolBinding:
    ...
    project_file_outputs: tuple[MCPProjectFileOutput, ...] = ()
    schema_version: int = MCP_CONFIG_SCHEMA_VERSION
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self,
            "tool_name",
            _clean_text(self.tool_name, field_name="tool_name", max_length=256),
        )
        object.__setattr__(
            self,
            "capability_id",
        ...
        object.__setattr__(
            self,
            "locality",
            _enum_value(self.locality, LocalityClass, field_name="locality"),
        )
        object.__setattr__(
            self,
            "cost_class",
            _enum_value(self.cost_class, CostClass, field_name="cost_class"),
        )
        ...
        normalized_features = tuple(
            validate_capability_id(value, field_name="feature")
        ...
        if any(
            not isinstance(item, MCPProjectFileInput)
            for item in self.project_file_inputs
        ):
            raise MCPConfigurationError(
                "project_file_inputs must contain MCPProjectFileInput values"
            )
        if not isinstance(self.project_file_outputs, tuple):
            raise MCPConfigurationError("project_file_outputs must be a tuple")
        if any(
            not isinstance(item, MCPProjectFileOutput)
            for item in self.project_file_outputs
        ):
            raise MCPConfigurationError(
                "project_file_outputs must contain MCPProjectFileOutput values"
            )
        input_names = [item.argument_name for item in self.project_file_inputs]
        output_names = [item.argument_name for item in self.project_file_outputs]
        if len(set(input_names)) != len(input_names):
            raise MCPConfigurationError(
                "project_file_inputs contains duplicate argument_name values"
            )
        if len(set(output_names)) != len(output_names):
            raise MCPConfigurationError(
                "project_file_outputs contains duplicate argument_name values"
            )
        overlap = set(input_names).intersection(output_names)
        if overlap:
            raise MCPConfigurationError(
                "project-file input/output argument names must be disjoint: "
                f"{sorted(overlap)!r}"
            )
    ...
    def from_dict(cls, data: Mapping[str, Any]) -> "MCPToolBinding":
        ...
        raw_file_outputs = data.get("project_file_outputs", [])
        ...
        if not isinstance(raw_file_outputs, list):
            raise MCPConfigurationError("project_file_outputs must be an array")
        return cls(
            schema_version=int(data.get("schema_version", MCP_CONFIG_SCHEMA_VERSION)),
            ...
            project_file_inputs=tuple(
                MCPProjectFileInput.from_dict(item) for item in raw_file_inputs
            ),
            project_file_outputs=tuple(
                MCPProjectFileOutput.from_dict(item) for item in raw_file_outputs
            ),
        )
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "project_file_inputs": [
                item.to_dict() for item in self.project_file_inputs
            ],
            "project_file_outputs": [
                item.to_dict() for item in self.project_file_outputs
            ],
        }
...
class MCPConfiguration:
    ...
    def __post_init__(self) -> None:
        ...
        for binding in self.bindings:
            if binding.profile_id not in known_profiles:
                raise MCPConfigurationError(
                    f"binding {binding.binding_id!r} references unknown profile "
                    f"{binding.profile_id!r}"
                )
    ...
    def from_dict(cls, data: Mapping[str, Any]) -> "MCPConfiguration":
        ...
        _exact_keys(
            data,
            {"schema_version", "profiles", "bindings"},
    ...
    def bindings_for(self, profile_id: str) -> tuple[MCPToolBinding, ...]:
        normalized = validate_capability_id(profile_id, field_name="profile_id")
        return tuple(
            binding for binding in self.bindings if binding.profile_id == normalized
        )
...
class MCPToolDescriptor:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(
            self,
            "name",
            _clean_text(self.name, field_name="tool name", max_length=256),
        )
        if self.title is not None:
            object.__setattr__(
                self,
                "title",
                _clean_text(self.title, field_name="tool title", max_length=500),
            )
        ...
        object.__setattr__(
            self,
            "input_schema",
            _schema(self.input_schema, field_name="input_schema") or {},
        )
        object.__setattr__(
            self,
            "output_schema",
            _schema(self.output_schema, field_name="output_schema"),
        )
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "output_schema": (
                None if self.output_schema is None else dict(self.output_schema)
            ),
        }
