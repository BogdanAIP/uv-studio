...
def _validate_generation_artifact(
    ...
    """Prove that durable metadata still describes the exact generated bytes."""

    attempt = job.current_attempt
    if attempt is None:
        raise GenerationJobError("generation materialization lost current attempt")
    generation = artifact.metadata.get("generation")
    if not isinstance(generation, dict):
...
def _mark_reconciled_success(
    ...
    *,
    ...
    """Finish the current attempt from proven durable local materialization evidence."""

    attempt = job.current_attempt
    if attempt is None:
        raise GenerationJobError("generation recovery lost current attempt")
    if job.status is GenerationStatus.SUCCEEDED:
        if attempt.output_reference_id != artifact_id or attempt.take_id != take_id:
            raise GenerationJobError("succeeded generation Job disagrees with durable materialization")
        return job
    if job.status not in {
        ...
        raise GenerationJobError(
            f"generation materialization cannot be reconciled from status {job.status.value!r}"
        )
...
def _reconcile_materialization(
    ...
    """Complete an interrupted Generation publication without provider replay.

    A current-attempt artifact may exist while the Job is still RUNNING or while an
    older buggy runtime already marked that same attempt FAILED/CANCELLED. Durable
    bytes/provenance win only after exact validation; otherwise recovery fails closed.
    """
    ...
    _validate_generation_artifact(manager, job, artifact)
    ...
    logger.warning(
        "generation job %s recovered as succeeded from verified durable artifact %s without provider replay",
        job.job_id,
        artifact.id,
    )
    return completed
...
def requeue_failed_generation_job(
    ...
    with manager.records.project_lock(project_id):
        ...
        if _matching_generation_artifacts(project, job):
            raise GenerationJobConflict(
                "failed generation attempt has a durable artifact pending recovery"
            )
        return _persist(
            manager,
            replace(
...
def recover_interrupted_project_jobs(
    ...
    current attempt whose exact artifact ProjectReference is already durable is
    completed locally from verified bytes/provenance, including terminal split state
    left by an older runtime after artifact publication.
    """
    ...
    with manager.records.project_lock(project_id):
        ...
        # Complete consequence-bearing Generation materialization that crossed the
        # durable ProjectReference boundary. Terminal FAILED/CANCELLED split states
        # from an older runtime are repaired here as well as RUNNING crash states.
        for job in manager.list(project_id):
            if job.status in {
                ...
                _reconcile_materialization(manager, production, job)
