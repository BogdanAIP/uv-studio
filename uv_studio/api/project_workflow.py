...
class RenderGeneralVideoActionRequest(_StrictActionRequest):
...
WorkflowActionRequest = (
    ...
    | RenderGeneralVideoActionRequest
...
def _validated_action_input(
    *,
    ...
    if state["recipe_id"] == "dubbing":
        ...
        if expected is not None:
            return _request_payload(request, expected, action_id=action_id)
    if state["recipe_id"] == "general_video" and action_id == "render_general":
        return _request_payload(request, RenderGeneralVideoActionRequest, action_id=action_id)
