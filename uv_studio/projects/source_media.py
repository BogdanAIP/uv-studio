...
class ProjectSourceMediaStore:
    ...
    def allocate(self, project_id: str, original_filename: str) -> AllocatedSourceMedia:
        ...
        try:
            absolute_path = self.project_store.resolve_project_file(project_id, relative_path, must_exist=False, allowed_roots=("sources",))
    ...
    def register(self, project_id: str, allocation: AllocatedSourceMedia, *, metadata: Mapping[str, Any]) -> ProjectDocument:
        ...
        reference = ProjectReference(id=allocation.source_id, kind="video", path=allocation.relative_path, metadata=dict(metadata))
        with self.project_store._lock:
            ...
            return self.project_store.update_project(project_id, sources=project.sources + (reference,))
        ...
        for reference in project.sources:
            if reference.id == source_id:
                if reference.kind != "video":
                    raise SourceMediaError(f"source reference {source_id!r} is not registered as video media")
                return reference
    ...
    def resolve(self, project_id: str, source_id: str) -> tuple[ProjectReference, Path]:
        ...
        try:
            path = self.project_store.resolve_project_file(project_id, reference.path, must_exist=True, allowed_roots=("sources",))
        except (ProjectValidationError, ProjectStoreError) as exc:
            raise SourceMediaError(str(exc)) from exc
    ...
    def resolve_verified(self, project_id: str, source_id: str) -> tuple[ProjectReference, Path]:
        reference, path = self.resolve(project_id, source_id)
        try:
